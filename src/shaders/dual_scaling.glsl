// Copyright © 2023 Cory Petkovsek, Roope Palmroos, and Contributors.

R"(

//INSERT: DUAL_SCALING_BASE
	// If dual scaling, apply to base texture
	if(region < 0) {
		matUV *= _multi_scale_macro_size;
	}
	albedo_ht = texture(_texture_array_albedo, vec3(matUV, float(out_mat.base)));
	normal_rg = texture(_texture_array_normal, vec3(matUV, float(out_mat.base)));
	if(out_mat.base == _multi_scale_texture || out_mat.over == _multi_scale_texture) {
		albedo_far = texture(_texture_array_albedo, vec3(matUV*_multi_scale_micro_size, float(_multi_scale_texture)));
		normal_far = texture(_texture_array_normal, vec3(matUV*_multi_scale_micro_size, float(_multi_scale_texture)));
	}

	float far_factor = clamp(smoothstep(_multi_scale_near, _multi_scale_far, length(v_vertex - v_camera_pos)), 0.0, 1.0);
	if(out_mat.base == _multi_scale_texture) {
		albedo_ht = mix(albedo_ht, albedo_far, far_factor);
		normal_rg = mix(normal_rg, normal_far, far_factor);
	}

//INSERT: UNI_SCALING_BASE
	albedo_ht = texture(_texture_array_albedo, vec3(matUV, float(out_mat.base)));
	normal_rg = texture(_texture_array_normal, vec3(matUV, float(out_mat.base)));

//INSERT: DUAL_SCALING_OVERLAY
		// If dual scaling, apply to overlay texture
		if(out_mat.over == _multi_scale_texture) {
			albedo_ht2 = mix(albedo_ht2, albedo_far, far_factor);
			normal_rg2 = mix(normal_rg2, normal_far, far_factor);
		}

)"